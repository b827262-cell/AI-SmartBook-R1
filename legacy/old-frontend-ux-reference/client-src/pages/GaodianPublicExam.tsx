...
export default function GaodianPublicExam() {
    ...
    try {
      ...
      rows.forEach((row) => {
        ...
        if (cells.length >= 4) {
          ...
          if (firstCellText === '編號' || secondCellText === '考試類組' || secondCellText.includes('考試科目')) {
            return;
          }
          
          const examCategory = cells[1]?.textContent?.trim() || '';
          ...
          if (examCategory && subject && downloadLink) {
            ...
            if (pdfUrl && !pdfUrl.startsWith('http')) {
              if (pdfUrl.startsWith('/')) {
                pdfUrl = `https://goldensun.get.com.tw${pdfUrl}`;
              } else {
                pdfUrl = `https://goldensun.get.com.tw/exam/${pdfUrl}`;
              }
            }
  ...
  const handleDownloadTxt = () => {
    ...
    // 生成 TXT 檔名
    let txtFilename = '高點公職考古題';
    if (examsToDownload.length === 1) {
      // 單個下載：考試類別_考試科目_年度.txt
      const exam = examsToDownload[0];
      const parts = [];
      if (exam.examCategory) parts.push(exam.examCategory);
      if (exam.subjectName) parts.push(exam.subjectName);
      if (exam.year) parts.push(`${exam.year}年`);
      txtFilename = parts.join('_') || '高點公職考古題';
    } else {
      // 批次下載：考試類別_年度_批次下載.txt（取第一個項目的資訊）
      const firstExam = examsToDownload[0];
      const parts = [];
      if (firstExam.examCategory) parts.push(firstExam.examCategory);
      if (firstExam.year) parts.push(`${firstExam.year}年`);
      parts.push('批次下載');
      txtFilename = parts.join('_') || '高點公職考古題_批次下載';
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto py-8">
        <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">高點公職考古題</h1>
        <p className="text-muted-foreground">
          從高點網站複製網頁原始碼，系統將自動解析 PDF 連結並生成下載檔案
        </p>
      </div>
      ...
      <div className="grid gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              按照以下步驟操作，快速批次下載考古題 PDF
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ol className="list-decimal list-inside space-y-2 text-sm">
              <li>前往 <a href="https://goldensun.get.com.tw/exam/List.aspx" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">高點公職考古題網站</a></li>
              <li>使用網站的搜尋功能，篩選您需要的考古題（例如：114年、土木工程）</li>
                  ...
                  <TableBody>
                    {parsedExams.map((exam, index) => (
                      <TableRow key={exam.id}>
                        ...
                        <TableCell>{exam.examCategory || '-'}</TableCell>
